...
import { toolConstraintsSummary, toolLibraryList, toolReadComponentSource, toolValidateSnippet, } from "./lib/workspace-tools.js";
import { buildAgentSystemPrompt } from "./prompts/system.js";
const server = new McpServer({
    name: "newgen-ui-agent",
    version: "1.0.0",
}, {
    instructions: "Tools and prompts for building React+Tailwind UI aligned with the newgen-ui package. Uses Groq cloud API (GROQ_API_KEY) for component codegen when you call newgen_agent_generate.",
});
server.registerTool("newgen_list_components", {
    description: "List exported components from the local newgen-ui package and the resolved filesystem root.",
}, async () => {
...
server.registerTool("newgen_read_component_source", {
    description: "Read the TypeScript source of a design-system component (reference for props and patterns).",
    inputSchema: {
        componentName: z
            .string()
...
server.registerTool("newgen_codegen_constraints", {
    description: "Return the hard constraints the AI agent and humans should follow when generating UI code.",
}, async () => ({
    content: [{ type: "text", text: toolConstraintsSummary() }],
}));
server.registerTool("newgen_validate_react_snippet", {
    description: "Lightweight static checks on a React+Tailwind code string (no AST).",
    inputSchema: {
...
server.registerTool("newgen_agent_generate", {
    description: "Run the Groq-powered agent to generate a React+Tailwind UI component. Set GROQ_API_KEY to use this tool.",
    inputSchema: {
        prompt: z
            .string()
        ...
        style: z
            .string()
            .optional()
            .describe("Desired visual style, e.g. modern, polished, production-ready"),
        quality: z
            .string()
            .optional()
...
server.registerPrompt("newgen_component_request", {
    description: "Structured request template for generating a newgen-ui style component.",
    argsSchema: {
