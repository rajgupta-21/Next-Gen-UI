...
function safeComponentName(raw: string): string | null {
  const cleaned = raw.replace(/\.tsx?$/i, "").replace(/[^a-zA-Z0-9]/g, "");
  if (!cleaned) return null;
  if (!EXPORTED_COMPONENTS.includes(cleaned as (typeof EXPORTED_COMPONENTS)[number])) {
    return null;
  }
  return cleaned;
}

export function toolLibraryList(): { names: string[]; packageRoot: string } {
  const packageRoot = getNewgenUiRoot();
  return { names: [...EXPORTED_COMPONENTS], packageRoot };
}

export function toolReadComponentSource(componentName: string): {
  ok: boolean;
  path?: string;
  content?: string;
  error?: string;
...
export function toolConstraintsSummary(): string {
  return [
...
function stripImportsAndExportsForSandbox(s: string): string {
  let out = s.trimStart();
  for (let i = 0; i < 40; i++) {
    const before = out;
 ...
 */
function repairNilJsxGarbage(s: string): string | null {
  if (!/<nil>/i.test(s) && !/\bfunction\s+nil\b/i.test(s)) return null;
...
function unescapeModelStringLiterals(s: string): string {
  return s
    .replace(/\\n/g, "\n")
    .replace(/\\r\n?/g, "\n")
    .replace(/\\t/g, "\t");
...
function wrapBareJsxIfNeeded(s: string): string {
  const t = s.trim();
  if (!/^<[A-Za-z!?]/.test(t)) return s;
  if (/^(export\s+)?(default\s+)?function\s+\w/m.test(t)) return s;
  if (/^const\s+\w+\s*[:=]/m.test(t)) return s;
  if (/^import\s+/m.test(t)) return s;
  return `export default function GeneratedComponent() {\n  return (\n    ${t}\n  );\n}`;
}
 ...
 */
function stripLowercaseHtmlHandlersFromTags(s: string): string {
  return s.replace(
    /<([a-zA-Z][a-zA-Z0-9]*)\b([^>]*?)(\/?>)/g,
    (full, tag: string, attrs: string, closer: string) => {
      let a = String(attrs);
 ...
 */
function repairTruncatedOpeningTags(s: string): string {
  let out = s;
  out = out.replace(
    /<button\b([^>]*className="[^"]*")\s*\r?\n\s*\}\s*\r?\n/gi,
    '<button$1 type="button">Click</button>\n',
  );
  out = out.replace(
    /<button\b([^>]*className="[^"]*")\s*\r?\n\s*(\s*)<\/div>/gi,
    '<button$1 type="button">Click</button>\n$2</div>',
...
export function sanitizeEmittedCode(raw: string): string {
...
export function toolValidateSnippet(code: string): {
  ok: boolean;
  issues: string[];
} {
  const issues: string[] = [];
